...
本项目没有使用 Hugo 常见的 `themes/` 目录来引用主题，而是将主题文件直接放到了项目根目录下。具体来说：

- 把 `themes/reimu/` 中的 `layouts/`、`assets/`、`static/`、`i18n/`、`data/` 移到了项目根目录
- 将主题的 `config/_default/params.yml` 与站点配置合并
- 删除了 `hugo.toml` 中的 `theme` 配置项（因为 Hugo 会直接在根目录找到所有文件）
- 所有配置文件统一为 YAML 格式

这样做只是为了方便直接修改主题代码，没有其他深意。
